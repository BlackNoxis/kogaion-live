...
LIVE_USER=${SABAYON_USER:-kogaionuser}
...
kogaion_setup_live_user() {
    ...
    if [ "${?}" != "0" ]; then
        ...
        useradd -d "/home/${live_user}" -g root -G ${live_groups} -c "Kogaion" \
...
kogaion_setup_gui_installer() {
    ...
    kogaion_setup_desktop_session "${LIVE_USER}" "fluxbox"
...
kogaion_fixup_gnome_autologin_session() {
    ...
    kogaion_setup_desktop_session "${usr}" "${cur_session}"
}
...
kogaion_is_normal_boot() {
    if ! kogaion_is_mce && ! kogaion_is_live_install && ! kogaion_is_steambox; then
        return 0
    else
        return 1
    fi
